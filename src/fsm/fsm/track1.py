#!/usr/bin/env python3
"""Target tracking service driven by YOLO detection results."""

import rclpy
from rclpy.node import Node
from rclpy.qos import HistoryPolicy, QoSProfile, ReliabilityPolicy

from uav_interfaces.msg import DetectionArray, TrackDetection
from uav_interfaces.srv import SetVelocity, TrackTarget


class YoloTrackingService(Node):
    def __init__(self):
        super().__init__('yolo_tracking_service')

        self.declare_parameter('yolo_topic', '/yolo/detections')
        self.declare_parameter('service_name', 'fsm/track_yolo_target')
        ...
        self.declare_parameter('tracking_duration_sec', 0.0)
        ...
        self.yolo_topic = self.get_parameter('yolo_topic').value                                      # YOLO 检测话题
        ...
        self.create_subscription(DetectionArray, self.yolo_topic, self._detections_cb, qos)            # 订阅YOLO检测数组话题
        ...
        service_name = self.get_parameter('service_name').value                                        # YOLO目标跟踪服务名称
        self.create_service(TrackTarget, service_name, self._handle_track_target)                      # 创建目标跟踪服务
        self.get_logger().info(f'YOLO tracking service ready: /{service_name}')                        # 记录YOLO跟踪服务已准备好
        self.get_logger().info(f'Subscribed YOLO topic: {self.yolo_topic}')                            # 记录已订阅的YOLO话题
        self.get_logger().info(f'Control enabled: {self.control_enabled}, velocity service: {self.velocity_service_name}') # 记录控制是否启用以及速度服务名称

    def _detections_cb(self, msg):  # 处理YOLO检测数组消息的回调函数
        ...
        self.latest_target = self._to_track_detection(target)
    ...
    @staticmethod
    def _to_track_detection(detection):  # 将YOLO检测结果转换为TrackDetection格式
        target = TrackDetection()
        target.track_id = 0
        target.class_id = detection.class_id
        target.score = detection.score
        target.x_min = detection.x_min
        target.y_min = detection.y_min
        target.x_max = detection.x_max
        target.y_max = detection.y_max
        target.age = 0
        target.hits = 0
    ...
    def _handle_track_target(self, request, response):  # 处理目标跟踪服务请求的回调函数
        ...
        if command == 'start':
            ...
            response.message = 'YOLO target tracking started'
        elif command == 'stop':
            self._stop_tracking()
            response.success = True
            response.message = 'YOLO target tracking stopped'
        else:
            response.success = True
            response.message = 'YOLO target tracking status'
    ...
    def _control_tick(self): # 定时器回调函数，用于控制无人机的速度
        ...
        if self._tracking_time_expired():
            self.get_logger().info(f'YOLO 跟踪时间到达 {self.tracking_duration_sec:.1f}s，停止跟踪')
            self._stop_tracking()
            return
    ...
    def _control_dt(self): # 计算两次控制之间的时间间隔
