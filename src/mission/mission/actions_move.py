...
class MoveClientAction:
    def __init__(self, node: Node, default_timeout: float = 30.0):
        self.node = node
        self.default_timeout = float(default_timeout)
        self.client = self.node.create_client(Move, 'uav/move')

    def execute(self, x: float, y: float, z: float, yaw: float = 0.0, timeout: float = None):
        if not self.client.wait_for_service(timeout_sec=5.0):
            return False, 'uav/move 服务不可用'
        ...
        if not fut.done():
            return False, f'调用 uav/move 超时（等待 {service_wait:.1f}s）'

        res = fut.result()
        if res is None:
            return False, 'uav/move 返回空'
